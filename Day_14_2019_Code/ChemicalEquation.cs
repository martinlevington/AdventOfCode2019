...
namespace Day_14_2019_Code
{
    public class ChemicalEquation : IEquation
    {
        public ChemicalEquation(string equation)
        {
            ...
            {
                throw new Exception("Error cannot make equation, incorrect number of parts");
            }

            Output = new Chemical(equationParts[1]);

            var makesParts = equationParts[0].Split(new[] {",", ", "}, StringSplitOptions.RemoveEmptyEntries);
